    ...
    - [] （验证模块的调研与设计）关于具身智能体的验证模块，given现在开源的框架里面很少有支持的，进一步调研一下，给一个验证模块儿基本功能设计和框架设计。搞清楚它到底要验证什么？需要什么样的信息作为输入，需要什么模型支撑，然后验证的输出应该是什么？planning模块和action模块的关系是什么。
        * Q1：现在是否真的没人这么做？如果有人这么做，他们的方法是什么？
            ...
            * 从目前已调研到的学术研究来看，对agent行为的验证主要包括：**任务规划的合理性**、**采取行动的合理性**、**行为结果的合理性**。
            - [x] TODOs：
                * [PhysiAgent](https://arxiv.org/pdf/2509.24524)：这篇文章做的是基于VLM+VLA的具身agent框架，利用VLM+VLA控制机械臂完成桌面操作任务。文章在VLM/VLA基座以外引入了monitor、reflector、memory、toolbox等模块，其中monitor的作用是评价VLA的工作是否完成；reflector作为二阶评测器，评价monitor的判断是否准确；此外，memory分为长期与短期两种记忆，短期记忆用来增强reflector，长期记忆用来增强planner。
                * ~~[AGENTGUARD](https://arxiv.org/pdf/2502.09809)~~：本文参考价值较低，只是一篇作业级别的原型报告。该文章设计了一个漏洞挖掘pipeline（本质是自动化安全测试 + 自动生成防御规则），通过三个不同的agent相互合作，挖掘当前工具连续执行中的风险项，并形成评估报告。评估报告后续可作为system prompt指导工作流程。
