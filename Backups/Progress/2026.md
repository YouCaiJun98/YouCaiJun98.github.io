    ...
    - [] （验证模块的调研与设计）关于具身智能体的验证模块，given现在开源的框架里面很少有支持的，进一步调研一下，给一个验证模块儿基本功能设计和框架设计。搞清楚它到底要验证什么？需要什么样的信息作为输入，需要什么模型支撑，然后验证的输出应该是什么？planning模块和action模块的关系是什么。
        * Q1：现在是否真的没人这么做？如果有人这么做，他们的方法是什么？
            ...
            * 在学术研究中，目前已有若干工作关注通过步骤检验来提高agent工作的准确性与任务完成率，目前已有的研究工作思路包括：
                * **引入judge / critic agent / LLM-as-a-Judge**来辅助任务规划等，如[arxiv2509](https://arxiv.org/pdf/2509.02761)这篇文章，使用Judge LLM（zero-shot prompt engineering）对Planner LLM指定的行动序列（Plan）进行批评，并让Planner迭代修正计划（简单循环流程设计）。TODO：[这篇文章VerifyLLM](https://arxiv.org/pdf/2507.05118)
                * **形式化验证**，将agent的行为转换为逻辑约束，如[arxiv2510](https://arxiv.org/abs/2510.12985)这篇文章，在semantic、plan、trajectory三个层面，对状态不变量、时序约束、安全条件进行验证，在任务执行前检测不安全计划。
                * **离线验证 + 运行时验证**，在agent执行具体的act前对待执行任务进行检查，如[arxiv2510](https://arxiv.org/abs/2510.05156)这篇文章。
                * **基于证据的验证**，解决agent声称完成但实际上没有完成的问题，如[arxiv2511](https://arxiv.org/pdf/2511.05524)这篇文章，要求agent必须提供artifact、run ID、metric等以证明真的完成了任务；
                * **面向人类对齐的验证**，事先定义人类标准，并以此初始化verifier agent，根据预定义的标准评估agent行为，如[arxiv2503](https://arxiv.org/abs/2503.12651)这篇文章。
        ...
        * Q1.5：agent系统执行发生错误时，潜在的原因有哪些？
            * TODO: 读[这篇文章](https://arxiv.org/pdf/2509.25370)
            * TODO：在具身场景下，还有什么其他的错误？
        * Q2：如果我们要自己做，它的逻辑和输入输出是什么？
            * （待细化）验证形式？（离线？在线？结合？ / 第三方验证？基于证据的验证？）验证对象？（任务规划？行为是否合法？行为是否完成？）
    - [] （benchmark的搭建与评估）有基本方案以后，基于[这个论文](https://arxiv.org/abs/2512.19021)里面的benchmark，里面提供很多VLN的场景和任务，应该也是基于isaacsim的，以及每个场景对应的SceneGraph，作为我们的基础（这样我们就不需要先手动构SceneGraph了），去验证一下想法。
        * （评估VLN论文里的benchmark，将它接到我们的框架里）
